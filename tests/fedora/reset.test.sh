...
RESET_OUTPUT=$(bash "$RIG_REPO/unix/fedora/libexec/rig-reset" 2>&1 || true)
...
HELP_OUTPUT=$(bash "$RIG_REPO/unix/fedora/libexec/rig-reset" --help 2>&1)
assert_output_contains "--help shows usage" "Usage:" echo "$HELP_OUTPUT"

section "Fedora rig reset - unknown subcommand"
UNKNOWN_OUTPUT=$(bash "$RIG_REPO/unix/fedora/libexec/rig-reset" invalid_subcommand 2>&1 || true)
